...
const MovieService = (() => {
    ...
    function normalizeMovie(raw) {
        ...
        movie.moods = Array.isArray(raw.moods) ? raw.moods : inferMood(raw.genre);
        movie.platforms = Array.isArray(raw.platforms) ? raw.platforms : pickById(id, PLATFORM_POOL, 2);
        movie.maturity = raw.maturity || inferMaturity(Number(raw.rating || 0));
        movie.content_warnings = Array.isArray(raw.content_warnings) ? raw.content_warnings : inferWarnings(raw.genre);
        ...
        movie.seasonal_tags = Array.isArray(raw.seasonal_tags) ? raw.seasonal_tags : inferSeasonalTags(raw);
        movie.crew = raw.crew || {
            writer: `${raw.director}`,
            music: 'Original Score Team'
        };
        movie.tags = Array.isArray(raw.tags) ? raw.tags : [];
