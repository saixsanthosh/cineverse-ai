...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        renderAll(state, currentUser, ui);
...
function injectDynamicSections() {
    const lastMoviesSection = [...document.querySelectorAll('.movies-section')].at(-1);
    if (!lastMoviesSection) return;
    ...
    blocks.forEach(block => {
        ...
        `;
        lastMoviesSection.after(section);
    });
}
...
function wireEvents(state, user, ui) {
    ...
    document.getElementById('exportBtn')?.addEventListener('click', () => {
        ...
        link.href = url;
        link.download = 'cineverse-backup.json';
        link.click();
...
function renderAll(state, user, ui) {
    const personalized = MovieService.getPersonalizedRecommendations(state.allMovies, user, 60);

    const filtered = MovieService.filterMovies(personalized, {
        query: state.query,
    ...
    renderGrid(ui.trendingContainer, trending.length ? trending : sorted.slice(0, 12), user);
    renderGrid(ui.popularContainer, topNetflix.length ? topNetflix : sorted.slice(0, 10), user);
    renderGrid(document.getElementById('mustWatchMovies'), mustWatch, user);
...
function movieCardTemplate(movie, user) {
    ...
    return `
        <div class="movie-card" data-movie-id="${movie.id}">
            <img src="${movie.poster}" alt="${movie.title}" class="movie-poster" onerror="this.src='data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 width=%22200%22 height=%22300%22><rect fill=%22%231e293b%22 width=%22200%22 height=%22300%22/><text x=%2250%%22 y=%2250%%22 fill=%22%2364748b%22 text-anchor=%22middle%22 dy=%22.3em%22No Image</text></svg>'">
...
function startReminderLoop(user, state) {
    const tick = () => {
        ...
        if (hour === Number(reminder.hour) && minute === 0) {
            ...
            const title = picks.length ? `Tonight pick: ${picks[0].title}` : 'Open CineVerse for picks';
            if ('Notification' in window && Notification.permission === 'granted') {
                new Notification('CineVerse Reminder', { body: title });
            } else {
                notifyToast(title);
            }
        }
